...
<template>
  ...
  >
    <div class="flex flex-col h-full gap-4">
      ...
      <!-- 进度条 -->
      <div v-if="appStore.progressLabel">
        <div class="flex justify-between text-xs text-gray-400 mb-1">
          <span>{{ appStore.progressLabel }}</span>
          <span>{{ appStore.progressPercent }}%</span>
        </div>
        <a-progress :percent="appStore.progressPercent / 100" :show-text="false" size="small" />
      </div>
      ...
      <div class="flex-1 overflow-y-auto">
        <div class="flex justify-between items-center mb-2">
          ...
          <a-button size="mini" type="text" @click="appStore.clearLogs()">清除</a-button>
        </div>
